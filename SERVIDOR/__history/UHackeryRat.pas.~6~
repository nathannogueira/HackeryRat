...
SS.Socket.Connections[0].SendText('<ERROR>'+'NÃO FOI POSSÍVEL ENVIAR O ARQUIVO');
fs.Free;
end;
end
else begin
SS.Socket.Connections[0].SendText('<ERROR>'+'O ARQUIVO NÃO EXISTE');
end;
end;
...
begin
   ...
   UFS:=TFileStream.Create(UploadTo,fmCreate or fmOpenWrite);
    ...
    begin
    ServerState:=sSIdle;
    SS.Socket.Connections[0].SendText('<RESULT>'+'UPLOAD DO ARQUIVO ' + UploadTo +' CONCLUÍDO...');
    UFs.free;
    sleep(1500);
    ...
    ufs.Free;
    SS.Socket.Connections[0].SendText('<RESULT>'+'NÃO FOI CONCLUÍDO O UPLOAD!');
end;
 end;
end;
end;
